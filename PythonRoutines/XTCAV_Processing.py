...
def two_gauss(x, mean_1, fwhm_1, height_1, mean_2, fwhm_2, height_2):
    return (gaussian(x, mean_1, fwhm_1, height_1),
            gaussian(x, mean_2, fwhm_2, height_2))
...
class XTCavProcessor:
    ...
    def set_data_source(self, ds):
        ...
        try:
            self.xtcav_streakV = psana.Detector('TCAV:BMP1:360:ADES')
        except KeyError:
            self.xtcav_streakV = None
        self.results = None
    ...
    def process(self, force_split=True, smoothing=20,
                use_abs=True, agr_thresh=0.5,
                fwhm_low_lim=1, fwhm_up_lim=40,
                streakV_thresh=38,
                verbose=False):
        self.n_processed += 1
        if self.xtcav_streakV is not None:
            voltage = self.xtcav_streakV(self.current_event)
            if not voltage > streakV_thresh:
                if verbose:
                    print('Streak voltage below threshold')
                return False
        if not self.event_ok:
            return False
        tpix, power, ok = self.s2s.XRayPower()
        t = np.array(tpix).astype(float)
        ...
        trace_centre_ind = np.abs(t - trace_centre).argmin()
        if verbose:
            print('Trace extends from {0} fs to {1} fs, centre at {2:.1f} fs'.format(
                t_lims[0], t_lims[1], trace_centre))

        window = 10
        mid_power = power[trace_centre_ind-window/2:trace_centre_ind+window/2]
        thr = np.mean(mid_power)
        power = power-thr
        power[power < 0] = 0
        ...
        (power_left, power_right) = (
            power[t <= trace_centre], power[t >= trace_centre])
        ...
        sum_l = power_left.sum()
        sum_r = power_right.sum()
        ...
        p0 = [moment_l, fwhm_l, power_left.max(),
              moment_r, fwhm_r, power_right.max()]

        try:
            opt_p, cov = optimize.curve_fit(
                double_gauss, t, power, bounds=bounds)
...
def process_run_mpi(ds_string, calib_dir, out_filename):
    ds = psana.DataSource(ds_string+':smd')
    proc = XTCavProcessor()
    proc.set_data_source(ds)
    psana.setOption('psana.calib-dir', calib_dir)

    out_filepath = os.path.join(
        '/reg/d/psdm/AMO/amolr2516/results/xtcav',
        out_filename+'.p')
    print 'Processing XTCAV trace for output file', out_filepath
    ...
    gathered_arrays = {key: comm.gather(array, root=0)
                       for key, array in arrays.items()}
    gathered_evt_index = comm.gather(evt_idx, root=0)

    if rank == 0:
        concatenated_arrays = {key: np.concatenate(
            array) for key, array in gathered_arrays.items()}
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Process XTCav traces from a single run (MPI enabled)')
    parser.add_argument(
        'datasource', help='Data source string (e.g. exp=AMO/amolr2516:run=55)')
    parser.add_argument('-o', dest='output', help='Output file name')
    args = parser.parse_args()
    if args.output is None:
        exp_matches = re.findall('exp=[0-9a-z]+', args.datasource)
        run_matches = re.findall('run=[0-9]+', args.datasource)
        if not run_matches or not exp_matches:
            raise ValueError(
                'Could not generate automatic file name from datasource string {0}'.format(
                    args.datasource))
    ...
    process_run_mpi(args.datasource,
                    '/reg/d/psdm/AMO/amolr2516/calib', out_file)
